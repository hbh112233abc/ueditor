    ...
    public function index()
    {
        ...
        $input  = [
            Elm::number(
                'max_image_size',
                '上传图片限制',
                $config['max_image_size']['value']
            )->min(10240)
                ->max(512000000)
                ->step(10240)
                ->info($config['max_image_size']['remark']),
            Elm::number(
                'max_file_size',
                '上传文件限制',
                ...
                ->max(512000000)
                ->step(10240)
                ->info($config['max_file_size']['remark']),
            Elm::number(
                'max_vedio_size',
                '上传视频限制',
                $config['max_vedio_size']['value']
            )->min(10240)
                ->max(5120000000)
                ->step(10240)
                ->info($config['max_vedio_size']['remark']),
            Elm::select(
                'thumb_type',
                '缩略图类型',
                intval($config['thumb_type']['value'])
            )->options(
                [
            ...
            Elm::number(
                'thumb_image_quality',
                '缩略图质量',
                $config['thumb_image_quality']['value']
            )->min(50)
                ->max(100)
                ->info($config['thumb_image_quality']['remark']),
            Elm::number(
                'thumb_max_width_height',
                '缩略图最长宽(高)',
                $config['thumb_max_width_height']['value']
            )->min(10)
                ->info($config['thumb_max_width_height']['remark']),
            ...
            )->info($config['water']['remark'])
                ->value(intval($config['water']['value'])),
            Elm::input(
                'water_text',
                '文字水印',
                $config['water_text']['value']
            )->info($config['water_text']['remark']),
            ...
            )->info($config['water_position']['remark'])
                ->value(intval($config['water_position']['value'])),
            Elm::uploadImage(
                'water_image',
                '图片水印',
                '/ueditor/setting/upload_imgage',
                $config['water_image']['value']
            )->uploadName('water_image')
                ->info($config['water_image']['remark']),
            Elm::input(
                'upload_field_name',
                '上传表单字段名称',
                $config['upload_field_name']['value']
            )->maxlength(16)
                ->required()
                ->info($config['upload_field_name']['remark']),
            Elm::input(
                'session_uid_key',
                'session用户id标识',
                $config['session_uid_key']['value']
            )->maxlength(16)
                ->required()
                ->info($config['session_uid_key']['remark']),
            Elm::input(
                'super_admin_uid',
                'session管理员id',
                $config['super_admin_uid']['value']
            )->maxlength(16)
                ->required()
                ->info($config['super_admin_uid']['remark']),
            Elm::input(
                'filesystem_type',
                '文件系统存储类型',
                $config['filesystem_type']['value']
            )->info($config['filesystem_type']['remark'])
                ->readonly(true),
            Elm::input(
                'filesystem_root',
                '文件系统存储路径',
                $config['filesystem_root']['value']
            )->info($config['filesystem_root']['remark'])
                ->required(),
            Elm::input(
                'filesystem_url',
                '文件系统访问路径',
                $config['filesystem_url']['value']
            )->info($config['filesystem_root']['remark'])
                ->required(),
        ];
        //创建表单
